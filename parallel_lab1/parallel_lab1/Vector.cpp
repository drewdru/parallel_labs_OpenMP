...
void FillingMas(double** mas, int n);//Заполнение массива
void FillingMasParallel(double** mas, int n, int thread);//Заполнение массива параллельно

void SumAB(double** masA, double** masB,double** masC, int n);//Сложение двух массивов
void SumABParallelFor(double** masA, double** masB,double** masC, int n, int thread);//Сложение двух массивов параллельно FOR
void SumABParallelSection(double** masA, double** masB, double** masC, int n, int thread);//Сложение двух массивов параллельно section

double SumC(double** masC, int n);//сумма элементов массива
double SumCCritical(double** masC, int n, int thread);//сумма элементов массива параллельно с критической секцией
double SumCReductor(double** masC, int n, int thread);//сумма элементов массива параллельно с редуктором
...
void FillingMas(double** mas, int n)//принимает массив и его размерность
{
    for(int i = 0; i<n; i++ )
    {
        for(int j = 0; j<n; j++)
...
void FillingMasParallel(double **mas, int n, int thread)//принимает массив, его размерность и количество потоков
{
...
*принимает два массива и складывает их
*результат помещается в третий массив, n - размерность массива
*/
{
    for(int i = 0; i<n; i++)
    {
...
double SumC(double** masC, int n)//принимает массив и его размерность
{
    double sum = 0;
    for(int i = 0; i<n; i++ )
    {
...
double SumCCritical(double** masC, int n, int thread)//принимает массив, его размерность и количество потоков
{
    omp_set_dynamic(0);      // Не менять число потоков во время исполнения
    omp_set_num_threads(thread);
    double sum = 0;
...
double SumCReductor(double** masC, int n, int thread)//принимает массив, его размерность и количество потоков
{
    omp_set_dynamic(0);      // Не менять число потоков во время исполнения
    omp_set_num_threads(thread);
    double sum = 0;
